/**
 * Client-side strip of risk bullets and score-card reasons that imply missing
 * land/zoning data, unavailable sources, or comparable/exit-price uncertainty.
 * Keep logic aligned with `artifacts/api-server/src/lib/risk-summary.ts`.
 */
...
function isComparableDataReliabilityRiskBullet(text: string): boolean {
  if (typeof text !== "string" || !text.trim()) return false;
  const s = text.toLowerCase();
...
/** Property-card score reasons — same suppression as server `filterScoreReasonStrings`. */
